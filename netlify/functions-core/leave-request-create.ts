...
const createLeaveRequest = async (event: AuthenticatedEvent) => {
  ...
  try {
    ...
    if (!['day', 'night'].includes(shift_type)) {
      return errorResponse('Invalid shift_type. Must be "day" or "night"', 400);
    }
    ...
    if (is_hourly_leave) {
      ...
      // Validate minimum 30 minutes (after lunch break calculation)
      if (calculatedLeaveMinutes < 30) {
        return errorResponse('Hourly leave must be at least 30 minutes (excluding lunch break)', 400);
      }
    ...
    const leaveTypeInfo = await query(
      'SELECT code, name_en, allow_hourly_leave FROM leave_types WHERE id = $1',
      [leave_type_id]
    );
    ...
    if (isUnlimitedLeave) {
      ...
      logger.log(`💳 [UNLIMITED] Current days taken: ${balance.used_days || 0}`);
    ...
    // ===== BALANCE MANAGEMENT =====
    if (isUnlimitedLeave) {
      // ✅ UNLIMITED LEAVE: Always increment the counter (no pending concept)
      if (initialStatus === 'pending' || initialStatus === 'approved') {
        logger.log(`💳 [UNLIMITED] Incrementing counter by ${total_days} days`);

        // For UNLIMITED leave, increment used_days to track total taken (total_days stays at 999)
        await query(
        ...
        logger.log(`✅ [UNLIMITED] Counter incremented: now tracking ${(balance.used_days || 0) + total_days} days taken`);
      }
    } else {
