...
const getDashboardStats = async (event: AuthenticatedEvent) => {
  ...
  try {
    ...
    `;
    const leaveStats = await query(leaveStatsSql, [userId, currentYear]);

    // ===== 2. สถิติการสลับวัน (Shift Swap Requests) =====
    const shiftStatsSql = `
      SELECT 
        COUNT(*) FILTER (WHERE status = 'pending') as pending_shifts,
        COUNT(*) FILTER (WHERE status = 'approved') as approved_shifts,
        COUNT(*) FILTER (WHERE status = 'rejected') as rejected_shifts,
        COUNT(*) as total_shifts
      FROM shift_swap_requests
      WHERE employee_id = $1 
        AND EXTRACT(YEAR FROM created_at) = $2
    `;
    const shiftStats = await query(shiftStatsSql, [userId, currentYear]);
    ...
    `;
    const leaveBalances = await query(leaveBalanceSql, [userId, currentYear]);
    ...
    if (['manager', 'hr', 'admin'].includes(userRole || '')) {
      ...
      `;
      const pendingShiftsResult = await query(
        pendingShiftsSql,
        userRole === 'manager' ? [userId] : []
      );
      ...
      pendingApprovals = {
        ...
        shift_requests: parseInt(pendingShiftsResult[0]?.count || '0'),
        total: parseInt(pendingLeavesResult[0]?.count || '0') + 
               parseInt(pendingShiftsResult[0]?.count || '0'),
      };
    }
    ...
    const recentShiftsSql = `
      ...
      LIMIT 5
    `;
    const recentShifts = await query(recentShiftsSql, [userId]);
    ...
    return successResponse({
      ...
      // สถิติการสลับวัน
      shift_stats: {
        total: parseInt(shiftStats[0]?.total_shifts || '0'),
        pending: parseInt(shiftStats[0]?.pending_shifts || '0'),
        approved: parseInt(shiftStats[0]?.approved_shifts || '0'),
        rejected: parseInt(shiftStats[0]?.rejected_shifts || '0'),
      },
