...
async function canIssueWarning(event: AuthenticatedEvent): Promise<boolean> {
  ...
  // Admin and HR can always issue warnings
...
async function generateNoticeNumber(): Promise<string> {
  ...
  const yearBE = year + 543; // Buddhist Era;

  // Get the maximum sequence number for this year
  // Use a simpler approach to avoid parameter type issues
  const result = await sql`
    SELECT
      COALESCE(MAX(CAST(SUBSTRING(notice_number FROM '^WN-[0-9]+-([0-9]+)$') AS INTEGER)), 0) as max_sequence
  ...
  return `WN-${yearBE}-${nextSequence.toString().padStart(4, '0')}`;
}
...
const handler: Handler = requireAuth(async (event: AuthenticatedEvent) => {
  ...
  try {
    ...
    // Retry logic for handling race conditions in notice number generation
    let noticeNumber;
    const maxRetries = 3;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        noticeNumber = await generateNoticeNumber();

        // Get warning expiry months from settings
        const [expiryMonthsSetting] = await sql`
            SELECT setting_value FROM warning_system_settings WHERE setting_key = 'warning_expiry_months'
          `;
        const expiryMonths = parseInt(expiryMonthsSetting?.setting_value || '12', 10);

        // Calculate expiry date and auto_inactive_date (12 months from effective date)
        ...
        // Determine if HR approval is required
        const requiresHRApproval = body.warning_type === 'SUSPENSION' || body.warning_type === 'TERMINATION';
        // All warnings start as PENDING_ACKNOWLEDGMENT until employee acknowledges
        const initialStatus = 'PENDING_ACKNOWLEDGMENT';
        const hrApprovalStatus = requiresHRApproval ? 'PENDING_HR_REVIEW' : null;

        // Prepare attachments array - convert to PostgreSQL text array format
        const attachmentsArray = body.attachments_urls && body.attachments_urls.length > 0
          ? body.attachments_urls
          : null;
        ...
        const [result] = await sql`
          ...
          ${body.incident_location || null},
          ${body.penalty_description},
          ${body.suspension_days || 0},
          ${body.suspension_start_date || null},
          ${body.suspension_end_date || null},
          ${body.effective_date},
          ${expiryDate.toISOString().split('T')[0]},
          true,
          ${autoInactiveDate.toISOString().split('T')[0]},
          ${requiresHRApproval},
          ${hrApprovalStatus},
          ${attachmentsArray},
          ${initialStatus}
      ...
      ` as any;
        ...
        if (body.witnesses && body.witnesses.length > 0) {
          for (const witness of body.witnesses) {
            ...
            INSERT INTO warning_witnesses (
              ...
              ${warningNotice.id},
              ${witness.witness_employee_id || null},
              ${witness.witness_name || null},
              ${witness.witness_position || null},
              ${witness.statement_th || null},
              ${witness.statement_en || null}
            )
          `;
          }
        }
        ...
        // Log audit trail
        const notesTh = requiresHRApproval ? 'สร้างใบเตือน รอ HR อนุมัติ' : 'สร้างใบเตือน';
        const notesEn = requiresHRApproval ? 'Warning notice created, pending HR approval' : 'Warning notice created';
        await sql`
      ...
      `;
        ...
        // Create notification for employee (only if not pending HR approval)
        ...
        `;
          ...
          // Create notification for HR to review
        ...
        console.warn(`⚠️ Warning creation attempt ${attempt} failed:`, insertError.message);
