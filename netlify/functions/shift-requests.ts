...
const getShiftRequests = async (event: AuthenticatedEvent) => {
  ...
  try {
    ...
    if (forApproval) {
      ...
      // ✅ HR/Admin เห็นทุก request รวมของตัวเอง (แต่อนุมัติของตัวเองไม่ได้ - ตรวจสอบใน canApprove)
      // Dept Admin/Manager กรอง request ของตัวเองออก
      if (role === 'hr' || role === 'admin') {
        console.log('🏢 HR/Admin: See ALL requests (including own), all departments');
        // ไม่กรองของตัวเองออก
      } else {
    ...
    if (forApproval && (role === 'hr' || role === 'admin' || is_department_admin || is_department_manager)) {
