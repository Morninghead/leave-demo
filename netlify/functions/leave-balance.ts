...
function processLeaveBalanceRow(row: any) {
  ...
  if (row.allow_hourly_leave) {
    ...
    const [thText, enText] = displayDaysNum > 0 && displayHoursNum > 0
      ? [
        `${displayDays} วัน ${displayHours} ชั่วโมง`,
        `${displayDays} day${displayDaysNum !== 1 ? 's' : ''} ${displayHours} hour${displayHoursNum !== 1 ? 's' : ''}`
      ]
      : displayDaysNum > 0 && displayHoursNum === 0
        ? [
          `${displayDays} วัน`,
          `${displayDays} day${displayDaysNum !== 1 ? 's' : ''}`
        ]
        : displayDaysNum === 0 && displayHoursNum > 0
          ? [
            `${displayHours} ชั่วโมง`,
            `${displayHours} hour${displayHoursNum !== 1 ? 's' : ''}`
          ]
          : ['0 ชั่วโมง', '0 hours'];
