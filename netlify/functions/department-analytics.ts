...
const getDepartmentAnalytics = async (event: AuthenticatedEvent) => {
  ...
  try {
    ...
    // Filter by start_date (when leave is taken)
    ...
    let monthCaseSql = 'lr.start_date';
    if (settings.cycle_type === 'day_of_month') {
      monthCaseSql = `CASE 
            WHEN EXTRACT(DAY FROM lr.start_date) >= ${settings.cycle_start_day} THEN lr.start_date + interval '1 month'
            ELSE lr.start_date 
          END`;
    }

    // 4. Monthly Department Trends (Top 5 departments)
    // Updated to use Fiscal Month Logic
    const monthlyDeptTrends = await query(
      ...
      SELECT
        ...
        d.name_th as department_name_th,
        d.name_en as department_name_en,
        EXTRACT(MONTH FROM (${monthCaseSql})) as month,
        TO_CHAR((${monthCaseSql}), 'Mon') as month_name,
        COUNT(lr.id) as requests,
        COALESCE(SUM(CASE WHEN lr.is_hourly_leave THEN lr.leave_minutes / 480.0 ELSE lr.total_days END), 0) as days
      FROM departments d
      JOIN employees e ON e.department_id = d.id
