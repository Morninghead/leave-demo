    ...
    const result = await query(
      ...
      WITH inserted_data AS (
        ...
        SELECT
          ...
          lp.leave_type_id,
          $1::integer,
          -- Calculate total_days: VAC/ANNUAL = tenure-based pro-rata, others = use policy default_days
          -- TENURE-BASED: Calculate from hire_date to CURRENT_DATE (not calendar year)
          -- ROUNDING: Round to nearest 0.5 day (e.g., 3.25 → 3.5, 3.74 → 3.5, 3.75 → 4.0)
          CASE
            WHEN lt.code IN ('ANNUAL', 'VAC') AND lp.default_days < 999 THEN
              -- PROBATION CHECK: If tenure < 119 days, entitlement is 0
              CASE 
                WHEN (CURRENT_DATE - e.hire_date::date) < 119 THEN 0
                ELSE
                  -- Calculate months worked from hire_date to CURRENT_DATE
                  LEAST(
                    ROUND(
                      (
                        (
                          ...
                          CASE
                            -- Thailand half-month rule: > 15 days = full month
                            WHEN EXTRACT(DAY FROM AGE(CURRENT_DATE, e.hire_date::date)) > 15 THEN 1
                            ELSE 0
                          END
                        ) / 12.0
                      ) * COALESCE(lp.annual_leave_after_probation, 6)::numeric * 2 -- × 2 for rounding
                    ) / 2.0, -- ÷ 2 to get 0.5 increments
                    COALESCE(lp.annual_leave_after_probation, 6)::numeric -- Cap at policy maximum
                  )
              END
            ELSE
          ...
          CASE
            WHEN lt.code IN ('ANNUAL', 'VAC') AND lp.default_days < 999 THEN
              -- PROBATION CHECK: If tenure < 119 days, minutes is 0
              CASE 
                WHEN (CURRENT_DATE - e.hire_date::date) < 119 THEN 0
                ELSE
                  ROUND(
                    LEAST(
                      ROUND(
                        ...
                        ) * COALESCE(lp.annual_leave_after_probation, 6)::numeric * 2
                      ) / 2.0,
    ...
    const result = await query(
      ...
      WITH inserted_data AS (
        ...
        SELECT
          ...
          $1::uuid,
          $2::integer,
          -- Calculate total_days: VAC/ANNUAL = tenure-based pro-rata, others = use default_days
          -- TENURE-BASED: Calculate from hire_date to CURRENT_DATE
          -- ROUNDING: Round to nearest 0.5 day
          CASE
