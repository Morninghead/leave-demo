 ...
 */
export async function ensureLeaveBalances(employeeId: string, year: number, forceUpdate: boolean = false): Promise<void> {
  ...
  for (const policy of policies) {
    ...
    if (policy.code === 'ANNUAL' || policy.code === 'VAC') {
      ...
      // 🔒 RESTRICTION: Employees on probation cannot have Annual Leave balance
      // [MODIFIED] Commented out to allow "Automatic Leave Balance Adjustment" visibility
      // User can now see their pro-rata balance even during probation.
      /*
      if (!probationStatus.isProbationComplete) {
        logger.log(`🚫 [PROBATION] Employee ${employeeId} has NOT completed probation (119 days). Setting Annual Leave balance to 0.`);
        totalDays = 0;
        proRataDays = 0;
      }
      */
    ...
    // PROTECTION: If balance exists and forceUpdate is false, SKIP this policy
    // This prevents accidental overwrites of existing data (User Request)
    if (isUpdate && !forceUpdate) {
      logger.log(`ℹ️ Balance exists for ${policy.code}, skipping update (forceUpdate=false)`);
      continue;
    }
