...
import { getEmailConfig } from './channel-config-helper';

// AWS SES Region (kept as env var since it's AWS-specific)
...
export async function sendEmail(options: EmailOptions): Promise<boolean> {
  try {
    // Get config from DB/env
    const emailConfig = await getEmailConfig();

    // Check if emails are enabled
    const alertsEnabled = await areEmailAlertsEnabled();
    if (!alertsEnabled && emailConfig.provider !== 'console') {
      logger.log('[Email] Alerts disabled, skipping email send');
      return false;
    }

    // Set defaults from config
    const from = options.from || emailConfig.senderEmail || 'noreply@company.com';
    const fromName = options.fromName || emailConfig.senderName || 'HR Leave System';
    const replyTo = options.replyTo || emailConfig.replyToEmail || 'hr@company.com';
    ...
    switch (emailConfig.provider) {
      ...
      case 'sendgrid':
        return await sendEmailViaSendGrid(options, from, fromName, replyTo, emailConfig.sendgridApiKey);

      case 'ses':
        return await sendEmailViaSES(options, from, fromName, replyTo);

      case 'smtp':
        return await sendEmailViaSMTP(options, from, fromName, replyTo, emailConfig);

      default:
        logger.error(`[Email] Unknown provider: ${emailConfig.provider}`);
        return false;
    }
  } catch (error) {
...
async function sendEmailViaSMTP(
  ...
  fromName: string,
  replyTo: string,
  emailConfig: { smtpHost: string; smtpPort: number; smtpUser: string; smtpPass: string }
): Promise<boolean> {
  try {
    if (!emailConfig.smtpHost) {
      logger.warn('[Email] SMTP host not configured');
      return false;
    }
