 ...
 * Note: For settings access, use requireHROnly instead (Admin cannot access settings)
 *
 * Usage:
...
/**
 * Require HR role only (excludes Admin)
 * Use this for settings and employee management
 *
 * Usage:
 * export const handler = requireHROnly(myHandler);
 */
export function requireHROnly(handler: (event: AuthenticatedEvent) => Promise<any>) {
  return requireRole(['hr'])(handler);
}

/**
 * Require manager, HR role for approvals (Admin EXCLUDED)
 * Note: Admin cannot approve leave/shift requests
 *
 * Usage:
 * export const handler = requireManager(myHandler);
 */
export function requireManager(handler: (event: AuthenticatedEvent) => Promise<any>) {
  return requireRole(['manager', 'hr'])(handler); // Admin removed - cannot approve
}
 ...
 * Requires HR role only (Admin EXCLUDED)
 *
 * Usage:
...
export function canManageEmployees(event: AuthenticatedEvent): boolean {
  return hasPermission(event, ['hr']); // Admin removed - cannot manage employees
}

/**
 * Check if user can approve leave requests
 * Requires Manager or HR role (Admin EXCLUDED)
 *
 * Usage:
...
export function canApproveLeave(event: AuthenticatedEvent): boolean {
  return hasPermission(event, ['manager', 'hr']); // Admin removed - cannot approve
}
...
export function canManageSettings(event: AuthenticatedEvent): boolean {
  return hasPermission(event, ['hr']); // Admin removed - cannot access settings
}
...
export function canViewReports(event: AuthenticatedEvent): boolean {
  return hasPermission(event, ['manager', 'hr', 'admin', 'dev']);
}
