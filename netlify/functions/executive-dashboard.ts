...
import { getFiscalSettings, getCurrentFiscalYearDateRange } from './utils/fiscal-year';
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (queryParams.startDate && queryParams.endDate) {
      ...
      const { start, end } = await getCurrentFiscalYearDateRange();
    ...
    // Get Fiscal Settings for Monthly Trends Grouping
    const fiskSettings = await getFiscalSettings();
    let monthCaseSql = `date_trunc('month', date_column)`;

    if (fiskSettings.cycle_type === 'day_of_month') {
      monthCaseSql = `CASE 
            WHEN EXTRACT(DAY FROM date_column) >= ${fiskSettings.cycle_start_day} 
    ...
    const monthlyTrendsResult = await query(
      ...
      LEFT JOIN (
        ...
        FROM (
          SELECT
            (
              ${monthCaseSql.replace(/date_column/g, 'created_at')}
            ) as month,
            COUNT(*) as requests,
    ...
    // FIXED: Aggregate at department level directly to avoid multiplication bug
    let departmentStats: DashboardData['departmentStats'] = [];
    try {
      const departmentStatsResult = await query(
        `SELECT
          ...
          COALESCE(leave_data.requests, 0) + COALESCE(swap_data.requests, 0) as requests,
          COALESCE(leave_data.days, 0) as days,
        ...
        ) leave_data ON d.id = leave_data.department_id
        LEFT JOIN (
          SELECT
            e.department_id,
          ...
          WHERE e.is_active = true
          GROUP BY e.department_id
        ) swap_data ON d.id = swap_data.department_id
        LEFT JOIN (
      ...
      departmentStats = departmentStatsResult
        .map(row => ({
          ...
          // Remove total_employees if it's not in the interface, or keep it if I update interface. 
          // Based on interface DashboardData at top of file, departmentStats has { department, requests, days }.
          // Logic usually wants to filter based on name validity.
        }))
        // Filter and map to match interface
        .map(row => ({
    ...
    if (parseInt(noBalanceResult[0]?.count || '0') > 0) {
      alerts.push({
        id: 'no-balance',
        message: `${noBalanceResult[0]?.count} employees have no leave balance set for ${startDate.getFullYear()}`,
        severity: 'warning'
      });
    }
