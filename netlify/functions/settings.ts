...
const settingsHandler = async (event: AuthenticatedEvent) => {
  ...
  if (event.httpMethod === 'GET') {
    try {
      ...
      const settings = {
        ...
        require_1_year_tenure_for_vacation: settingsObj.require_1_year_tenure_for_vacation === 'true',
  ...
  if (event.httpMethod === 'PUT') {
    ...
    // Cast to any to access potentially custom claims not in strict type
    const userAny = event.user as any;
    const isHR = userAny?.is_hr === true;
    const isDev = (userAny?.employeeCode === '999999999') || (userAny?.sub === '999999999') || (userAny?.employee_code === '999999999'); // Check exact ID, handle both cases

    const hasPermission = isHR || ['hr', 'admin'].includes(userRole || '') || isDev;
    ...
    try {
      ...
      const {
        ...
        branding_settings,
        require_1_year_tenure_for_vacation
      } = body;
      ...
      if (require_1_year_tenure_for_vacation !== undefined) {
        updates.push({ key: 'require_1_year_tenure_for_vacation', value: String(require_1_year_tenure_for_vacation) });
      }
