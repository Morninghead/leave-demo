...
const getLeaveUsageAnalytics = async (event: AuthenticatedEvent) => {
  ...
  try {
    ...
    // 1. Usage by Leave Type - Use start_date for leave analysis
    ...
    // 2. Monthly Trends - Use start_date for when leave is taken
    const monthlyTrends = await query(
      ...
      WHERE lr.start_date >= $1 AND lr.start_date <= $2 ${deptFilter}
      GROUP BY 
