...
const getShiftSwapRequests = async (event: AuthenticatedEvent) => {
  ...
  try {
    ...
    const userInfoResult = await query(
      `SELECT department_id, is_department_admin, is_department_manager, role, employee_code
       FROM employees
       WHERE id = $1`,
      [userId]
    );
    ...
    const { department_id: userDeptId, is_department_admin, is_department_manager, role, employee_code } = userInfo;
