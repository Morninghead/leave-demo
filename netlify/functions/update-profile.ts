...
const updateProfile = async (event: AuthenticatedEvent) => {
  ...
  try {
    ...
    const currentUserResult = await query(`SELECT * FROM employees WHERE id = $1`, [userId]);
    ...
    if (first_name_th !== undefined) {
      updates.push(`first_name_th = $${paramIndex++}`);
      values.push(first_name_th || null);
    }

    if (last_name_th !== undefined) {
      updates.push(`last_name_th = $${paramIndex++}`);
      values.push(last_name_th || null);
    }

    if (first_name_en !== undefined) {
      updates.push(`first_name_en = $${paramIndex++}`);
      values.push(first_name_en || null);
    }

    if (last_name_en !== undefined) {
      updates.push(`last_name_en = $${paramIndex++}`);
      values.push(last_name_en || null);
    }
