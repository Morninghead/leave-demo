...
const getMe = async (event: AuthenticatedEvent) => {
  ...
  try {
    ...
    logger.log('🔍 [AUTH-ME] Fetching user:', userId);

    // Query employee with department info
    // Use same column aliases as auth-login.ts for consistency
    let employees;
    try {
      ...
      // Fallback query without department_code if it doesn't exist
      logger.log('⚠️ [AUTH-ME] First query failed, trying fallback:', queryError.message);
      employees = await query<any>(
        `
        SELECT
          e.id,
    ...
    if (employees.length === 0) {
      logger.log('❌ [AUTH-ME] User not found:', userId);
      return errorResponse('User not found', 404);
    }

    const employee = employees[0];
    logger.log('✅ [AUTH-ME] User found:', employee.employee_code);
    ...
    // Note: national_id removed for security - rarely needed client-side
    const user = {
      id: employee.id || '',
    ...
    logger.error('❌ [AUTH-ME] Get user error:', {
